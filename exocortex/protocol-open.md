...
> **⚠️ При продолжении из summary (сжатый контекст):** Open протокол выполняется ПОЛНОСТЬЮ заново. Summary ≠ открытая сессия. Нет исключений.
...
## Синхронизация контекста недели (блокирующее)

**ОБЯЗАТЕЛЬНО. После WP Gate, перед Ритуалом.**

1. Прочитать `memory/project-brain.md` — архитектура, решения, статус агентов, история
2. Прочитать `DS-strategy/current/SESSION-CONTEXT.md`
2. Извлечь текущие статусы РП:
   - Секция "Что делаем прямо сейчас" → активный РП
   - Секция "Что сделано сегодня" → завершённые РП
3. Сравнить с таблицей РП в `MEMORY.md` (секция "РП текущей недели")
4. Если есть расхождения (статус, дедлайн) → обновить `MEMORY.md`
5. Если обновлял → коммит: `sync: MEMORY.md синхронизирован с SESSION-CONTEXT`

**Цель:** Агент всегда видит актуальную картину недели (N из M done, дедлайны, приоритеты).

---

## Ритуал согласования

> После синхронизации контекста, перед работой.
