...
1. `git commit` + `git push` в репо где были изменения
2. Обновить `DS-strategy/SESSION-CONTEXT.md` — что сделано, что осталось, следующий шаг
3. Обновить `MEMORY.md` если изменился статус РП

**SESSION-CONTEXT.md дублируется во все активные репо после каждой сессии.**
