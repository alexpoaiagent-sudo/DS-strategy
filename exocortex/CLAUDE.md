...
2. **Стартовый экран экзокортекса:** Любое открытие рабочей сессии начинается с русского экрана состояния среды. Цвета: зелёный = можно открывать день, жёлтый = только после явного подтверждения, красный = обычное открытие запрещено до устранения проблемы.
3. **Push:** Незапушенные коммиты → ОБЯЗАН спросить «Запушить?». Push ДО отчёта Закрытия.
4. **Pull-before-Commit (DS-strategy):** `git pull --rebase` → модификация → `commit` → `push`. Нарушение = конфликты.
5. **Без Obsidian (DS-strategy):** Просмотр через VS Code, НЕ Obsidian.
6. **Close:** При триггере Закрытия → протокол Закрытия → выполнить.
7. **Выбор модели:** Объявить в Ритуале согласования. Opus (архитектура, сложный код), Sonnet (типовые задачи), Haiku (поиск, простые фиксы). Детали → `memory/protocol-open.md`.
8. **Терминология VK-offee:** "Шеф-повар" = должность на кухне. "Шеф" / "для шефа" = руководитель проекта VK-offee.
9. **Внеплановые задачи:** Если во время выполнения РП пользователь даёт новую задачу → записать в `DS-strategy/inbox/INBOX-TASKS.md` → предложить продолжить текущий РП. Переход к новой задаче ТОЛЬКО после завершения текущего РП.
10. **Итерационная дисциплина рабочих продуктов (БЛОКИРУЮЩЕЕ):** Сложная работа ведётся только как цепочка отдельных рабочих продуктов. Один WP = один артефакт = одно truthful закрытие через `close-task.sh` = отдельный commit/push = обновлённый `SESSION-CONTEXT.md`. Нельзя открывать следующий WP, пока текущий не закрыт. Нельзя вести один длинный контекст из нескольких незакрытых WP. Если задача разрастается — остановиться, зафиксировать текущий WP, создать следующий WP и продолжить уже в новом цикле.
11. **РУССКОЕ НАЗВАНИЕ ФАЙЛА (БЛОКИРУЮЩЕЕ):** При создании ЛЮБОГО файла название ОБЯЗАТЕЛЬНО дублируется на русском языке в скобках. Формат: `ID-english-slug (Русское описание).md`. Примеры: `PARK.WP.022-letter-to-konstantin (Письмо архитектору Константину).md`, `KITCHEN.METHOD.001-ingredient-change (Замена ингредиента).md`. БЕЗ ИСКЛЮЧЕНИЙ. Нарушение = файл не создан.
12. **Отчёт сессии стратегирования (БЛОКИРУЮЩЕЕ):** При создании StrategyReport ОБЯЗАТЕЛЬНО включить ВСЕ задачи из `DS-strategy/inbox/INBOX-TASKS.md`. Разделить на две категории: (1) Действительные (можно разработать сейчас), (2) Ожидание (блокеры, внешние зависимости). Формат: карточки (не таблицы). Без этого отчёт неполный.
13. **Extractor лимит 5 captures (намеренный):** Ограничение на 5 captures за цикл — это намеренно, чтобы не раздувать план на неделю. Остальные задачи остаются в INBOX-TASKS и решаются по очереди. На новую неделю → переставляешь приоритеты вручную, выбирая из п��лного списка задач (не просто идёшь по очереди).
