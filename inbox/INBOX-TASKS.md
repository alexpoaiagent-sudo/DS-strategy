...
- [pending] 2026-03-21: Исправить автозапись SESSION-CONTEXT и вывод close-task.sh
  - Контекст: По итогам закрытия сессии выявлены косметические баги. В `SESSION-CONTEXT.md` записи в блоке «Что сделано сегодня» местами склеиваются в одну строку, а `close-task.sh` иногда печатает дублированный список репозиториев в строке «Запушено»
  - Приоритет: high
  - Что нужно сделать:
  ...
  - Артефакт: корректный формат `SESSION-CONTEXT.md` и чистый вывод `close-task.sh`
  - Репо: DS-strategy / корневой скрипт `~/Github/close-task.sh`
